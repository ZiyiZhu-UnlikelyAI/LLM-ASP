...
from babi_utils import ASP_solve, GPT_parse, get_query, process_answer_set, write_debug
...
parser.add_argument(
    ...
    type=str,
    help='Specifies model (supports "text-davinci-002" and "text-davinci-003")',
)
...
for task_idx in task_nums:
    ...
    for eval_num in pbar:
        ...
        # 2. Call GPT-3
        instance_string, prompt_cache = GPT_parse(
            task_prompts, context, query, gpt_model, config, prompt_cache
        )
        ...
        try:
            ...
            pred = process_answer_set(
                answer_set, config.answer_fact, config.post_processing, answer
            )
        except:
            pred = ""
        if pred == answer.lower():
            ...
            debug.append(
                [eval_num, context, query, instance_string, ASP_program, pred, answer]
            )
        total += 1
