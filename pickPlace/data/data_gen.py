...
class BlockWorldData:
    ...
    def parse_answer_sets(self, answer_sets, bowls):
        ...
        for answer_set in answer_sets:
            ...
            actions = [
                atom.replace("pick_and_place(", "")[:-1]
                for atom in answer_set
                if atom.startswith("pick_and_place")
            ]
            ...
            states = [
                atom.replace("on(", "")[:-1]
                for atom in answer_set
                if atom.startswith("on") and atom[-2].isdigit()
            ]
            ...
            for i, action in enumerate(actions):
                plan += (
                    f"{i+1}. Move the {action[0]} onto the {action[1]}.\n\n# State:\n"
                )
    ...
    def gen_data(
        ...
        random.seed(seed)
        data, visited_states, stack_fact = (
            [],
            set(),
            f"#const max_height={max_height}.\n",
        )
        for _ in range(max_trial):
            ...
            for type in ("init", "goal"):
                ...
                for block in blocks:
                    ...
                    available_indices = [
                        i for i in range(n_stacks) if count_blocks[i] < max_height
                    ]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--n_data", default=10, type=int, help="the number of data to be generated"
    )
    parser.add_argument(
        "--min_stack",
        default=3,
