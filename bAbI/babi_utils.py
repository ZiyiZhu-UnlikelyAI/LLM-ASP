...
def get_response(prompt, model, stop=None, temp=0.0, max_tokens=30, sleep=None):
    ...
    response = openai.Completion.create(
        model=model, prompt=prompt, temperature=temp, max_tokens=max_tokens, stop=stop
    )
    ...
    for context_line in context:
        if context_line in prompt_cache:
            ...
            response = get_response(
                context_prompt + context_line + context_parse_word,
                model,
                stop=stop_context,
                sleep=sleep,
            )
    ...
    if separate == True:
        ...
        if query in prompt_cache:
            ...
            query_response = get_response(
                query_prompt + query + query_parse_word,
                model,
                stop=stop_query,
                sleep=sleep,
            )
        ...
        query_text = ""
    instance_string = create_extra_rules(
        facts, query_text, temporal_stamp, separate=separate
    )
...
def GPT_parse_all(
    ...
    context_query = (
        context_preprocess(context_query) if context_preprocess else context_query
    )
    ...
    if separate:
        ...
        if query in prompt_cache:
            ...
            query_response = get_response(
                query_prompt + query + query_parse_word, model, stop=stop, sleep=sleep
            )
            prompt_cache[query] = query_response
    ...
    if not temporal_stamp:
        ...
        instance_string = create_extra_rules(
            facts[:-1], facts[-1], temporal_stamp, separate=True
        )
    return instance_string, prompt_cache
...
def GPT_parse(prompts, context, query, model, config, prompt_cache):
    ...
    if config.custom_parse:
        instance_string, prompt_cache = config.custom_parse(
            prompts,
            context,
            query,
            model,
        ...
        return instance_string, prompt_cache
    if config.iterative:
        instance_string, prompt_cache = GPT_parse_iterative(
            prompts,
            context,
            config.context_preprocess,
            query,
            model,
            config.stops,
            config.parse_word,
            config.sleep,
            config.temporal_stamp,
            config.separate,
            prompt_cache,
        )
    else:
        instance_string, prompt_cache = GPT_parse_all(
            prompts,
            context,
            config.context_preprocess,
            query,
...
def custom17(prompts, context, query, model, stops, parse_word, sleep, prompt_cache):
    ...
    if full_context in prompt_cache:
        ...
        response = get_response(
            context_prompt + full_context + parse_word[0],
            model,
            stop=context_stop,
            max_tokens=100,
            sleep=sleep,
    ...
    if query_GPT in prompt_cache:
        ...
        query_response = get_response(
            query_prompt + parse_word[0] + query_GPT + parse_word[1],
            model,
            stop=query_stop,
            max_tokens=60,
            sleep=sleep,
